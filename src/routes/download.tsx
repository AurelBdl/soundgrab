...
function DownloadPage() {
	...
	const [trackPreviews, setTrackPreviews] = useState<Map<string, TrackPreview>>(
		new Map(),
	);
	...
	const handleDownloadSingle = async (url: string) => {
		...
		try {
			...
			alert(error instanceof Error ? error.message : "Error downloading");
		}
	};
			...
			for (const url of validUrls) {
				try {
					...
					if (result.success && result.downloadUrl && result.trackTitle) {
						...
						const blob = await response.blob();
						const sanitizedTitle = result.trackTitle
							.replace(/[<>:"/\\|?*]/g, "")
							.trim();
	...
	return (
		<div className="flex-1">
			<div className="mx-auto flex w-full max-w-6xl flex-col gap-6 lg:mx-0 lg:max-w-none">
